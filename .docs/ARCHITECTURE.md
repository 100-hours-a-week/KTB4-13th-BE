...
- Domain은 Application, API, Infrastructure, Spring, JPA, Jackson을 참조하지 않습니다.
- Infrastructure는 Port를 구현하며 Domain과 Entity·외부 DTO를 변환합니다.
...
| Domain | domain/{Feature} | 상태·불변식·업무 행위 |
| 저장 Port | application/port/{Feature}Repository | 저장·조회 계약 |
| JPA Entity | infrastructure/persistence/entity/{Feature}Entity | 테이블 매핑 |
...
| 영속성 변환 | infrastructure/persistence/mapper/{Feature}PersistenceMapper | Domain ↔ Entity |
| 외부 Port | application/port/{Feature}Client | 외부 기능 계약 |
| 외부 구현 | infrastructure/client | 외부 DTO·SDK·타임아웃·오류 변환 |

- 단순 HTTP 변환은 `Request.toCommand()`, `Response.from(Result)`에 둡니다.
- 복잡하거나 재사용되는 변환만 별도 converter로 분리합니다.
- Domain은 Entity·HTTP DTO를 알지 못합니다. Domain·Entity를 HTTP 응답으로 직접 반환하지 않습니다.
...
공통 오류 계약은 `common/exception/BusinessException`과 `ErrorCode`입니다.
기능별 오류 코드는 `core/{feature}/domain/exception`에 둡니다.
JPA·외부 SDK 예외를 공통 계약으로 변환하고 `support/web/ApiExceptionHandler`가 HTTP 상태와 응답을 매핑합니다.
민감한 오류 원문이나 인증정보를 응답·로그에 노출하지 않습니다.

## 6. 공통 코드와 접근 수준

- 여러 기능에서 의미와 규칙이 같은 기술 중립 코드만 common에 둡니다.
- HTTP 공통 처리는 support/web, 공유 애플리케이션 설정은 support/config에 둡니다.
...
- Persistence: Mapper·기술 예외 변환·MySQL 쿼리·Flyway·DB 제약 검증
...
실행 JAR는 `./gradlew bootJar`로 생성합니다. 테스트는 Docker가 없다고 자동 생략하지 않습니다.
API 변경은 DTO·spec·Controller 테스트, DB 변경은 Domain·Entity·Port·Mapper·마이그레이션을 함께 확인합니다.
