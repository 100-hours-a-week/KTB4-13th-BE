...
| `productId` | null이 아니고 양수 | `CartAddRequest.java:8-9` |
| `quantity` | 생략하면 1, 입력하면 1~500 | `CartAddRequest.java:8-15` |
| 성공 응답 | `200`, `{ "success": true }` | `CartController.java:26-31` |
| 잘못된 요청 | 공통 `E400` 응답 | `CartAddCommand.java:5-10`, `CartItem.java:60-76` |
...
- 위치: `src/main/java/com/book/core/cart/domain/Cart.java:57-66`, `CartItem.java:79-82`
...
- API 입력: `CartAddRequest.java:8-15`
- Application 입력: `CartAddCommand.java:5-10`
- Domain 불변식: `CartItem.java:73-76`
...
UseCase는 장바구니를 잠근 뒤 Domain에 추가를 위임하고, 결과 항목만 저장합니다.

1. `CartAddUseCase.execute()`가 쓰기 트랜잭션을 시작합니다.
2. `CartRepository.lockOrCreate()`가 사용자별 장바구니를 없으면 생성하고 비관적 쓰기 잠금으로 읽습니다.
...
- 잠금·생성: `CartJpaRepository.java:12-23`, `CartRepositoryImpl.java:20-32`
- 항목 저장: `CartRepositoryImpl.java:34-55`
- 이유: 같은 사용자의 동시 첫 추가에서 장바구니가 여러 개 생기거나, 같은 상품이 중복 insert되는 것을 막기 위함입니다.
...
| `CartAddRequest` | HTTP 필드 검증과 quantity 기본값 처리 | `src/main/java/com/book/core/cart/api/request/CartAddRequest.java:8-15` |
| `CartControllerSpec` | springdoc API 계약과 대체 정책 문서화 | `src/main/java/com/book/core/cart/api/spec/CartControllerSpec.java:18-33` |
| `CartAddCommand` | HTTP 밖에서도 사용할 추가 전제조건 검증 | `src/main/java/com/book/core/cart/application/command/CartAddCommand.java:5-10` |
| `CartAddUseCase` | 트랜잭션 안에서 Port와 Domain을 조율 | `src/main/java/com/book/core/cart/application/usecase/CartAddUseCase.java:11-19` |
| `Cart` | 동일 상품 대체, 상품 수 제한, 항목 상태 관리 | `src/main/java/com/book/core/cart/domain/Cart.java:26-74` |
| `CartItem` | 상품 ID·수량 불변식과 수량 대체 값 생성 | `src/main/java/com/book/core/cart/domain/CartItem.java:23-82` |
| `CartRepository` | add에 필요한 장바구니 잠금·항목 저장 최소 계약 | `src/main/java/com/book/core/cart/application/port/CartRepository.java:6-9` |
| `CartRepositoryImpl` | JPA 잠금, 기존 항목 로드, 저장, 공통 저장소 오류 변환 | `src/main/java/com/book/core/cart/infrastructure/persistence/repository/CartRepositoryImpl.java:16-70` |
...
| `CartControllerTest` | quantity 기본값 1, 입력 수량 전달, 잘못된 userId·productId·quantity의 400 및 UseCase 미호출 | `src/test/java/com/book/core/cart/api/CartControllerTest.java:22-87` |
...
전체 `integrationTest`와 `check`는 PR 생성 전에 다시 실행하고 결과를 PR 본문에 기록합니다.
