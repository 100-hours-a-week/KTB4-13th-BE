...
class SampleControllerTest {
    ...
    @MockitoBean SampleCreateUseCase createUseCase;
    @MockitoBean SampleQueryUseCase queryUseCase;
    ...
    void 생성_요청을_Command로_변환하고_201과_Location을_응답한다() throws Exception {
        ...
        mvc.perform(MockMvcRequestBuilders.post("/api/v1/samples")
                        .contentType(MediaType.APPLICATION_JSON).content("{\"name\":\"  책  \"}"))
    ...
    void 누락_null_빈문자열_잘못된_JSON을_거부한다(String body) throws Exception {
        mvc.perform(MockMvcRequestBuilders.post("/api/v1/samples")
    ...
    void 정규화_후의_이름_길이를_검증한다() throws Exception {
        ...
        mvc.perform(MockMvcRequestBuilders.post("/api/v1/samples").contentType(MediaType.APPLICATION_JSON)
                        .content("{\"name\":\"  " + name + "  \"}"))
    ...
    void 잘못된_ID는_400을_응답한다(String id) throws Exception {
    ...
    void 저장소_오류의_상세_원인은_HTTP_응답에_노출하지_않는다() throws Exception {
        when(queryUseCase.execute(any())).thenThrow(new BusinessException(CommonErrorCode.STORAGE_FAILURE,
